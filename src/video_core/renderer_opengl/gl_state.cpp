...
OpenGLState::OpenGLState() {
    ...
    texture_shadow_cube_unit.texture_cube = 0;
    texture_shadow_cube_unit.sampler = 0;
...
void OpenGLState::Apply() const {
    ...
    if (texture_shadow_cube_unit.texture_cube != cur_state.texture_shadow_cube_unit.texture_cube) {
        glActiveTexture(TextureUnits::TextureShadowCube.Enum());
        glBindTexture(GL_TEXTURE_CUBE_MAP, texture_shadow_cube_unit.texture_cube);
    }
    if (texture_shadow_cube_unit.sampler != cur_state.texture_shadow_cube_unit.sampler) {
        glBindSampler(TextureUnits::TextureShadowCube.id, texture_shadow_cube_unit.sampler);
    }
...
OpenGLState& OpenGLState::ResetTexture(GLuint handle) {
    ...
    if (texture_shadow_unit.texture_2d == handle)
        texture_shadow_unit.texture_2d = 0;
    if (texture_shadow_cube_unit.texture_cube == handle)
        texture_shadow_cube_unit.texture_cube = 0;
...
OpenGLState& OpenGLState::ResetSampler(GLuint handle) {
    ...
    if (texture_shadow_cube_unit.sampler == handle) {
        texture_shadow_cube_unit.sampler = 0;
    }
    return *this;
}
