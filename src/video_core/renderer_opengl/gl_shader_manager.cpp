...
void PicaUniformsData::SetFromRegs(const Pica::ShaderRegs& regs,
    ...
    std::transform(setup.uniforms.b.begin(), setup.uniforms.b.end(), bools.begin(),
                   [](bool value) -> BoolAligned { return {value ? GL_TRUE : GL_FALSE}; });
    std::transform(std::begin(regs.int_uniforms), std::end(regs.int_uniforms), i.begin(),
                   [](const auto& value) -> GLuvec4 {
                       return {value.x.Value(), value.y.Value(), value.z.Value(), value.w.Value()};
                   });
    std::transform(std::begin(setup.uniforms.f), std::end(setup.uniforms.f), f.begin(),
