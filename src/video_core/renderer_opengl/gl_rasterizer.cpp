...
static void SyncProcTexLUT128(const Pica::State::ProcTex::Lut128& lut,
                              std::array<GLuint, 128>& lut_data, GLenum texture) {
    std::array<GLuint, 128> new_data;
    ...
    if (new_data != lut_data) {
        ...
        glTexSubImage1D(GL_TEXTURE_1D, 0, 0, 128, GL_RED_INTEGER, GL_UNSIGNED_INT, lut_data.data());
    }
}

void RasterizerOpenGL::SyncProcTexNoiseLUT() {
    SyncProcTexLUT128(Pica::g_state.proctex.noise_table, proctex_noise_lut_data, GL_TEXTURE10);
}

void RasterizerOpenGL::SyncProcTexColorMap() {
    SyncProcTexLUT128(Pica::g_state.proctex.color_map_table, proctex_color_map_data, GL_TEXTURE11);
}

void RasterizerOpenGL::SyncProcTexAlphaMap() {
    SyncProcTexLUT128(Pica::g_state.proctex.alpha_map_table, proctex_alpha_map_data, GL_TEXTURE12);
}
